...
export const UserManagement = ({ onBack }: UserManagementProps) => {
  ...
  const { data: usersData, isLoading, error } = useQuery({
    ...
    queryFn: () => apiClient.getAllUsers(currentPage, pageSize),
  });

  const deleteUserMutation = useMutation({
    mutationFn: (userId: string) => apiClient.deleteUser(userId),
    onSuccess: (response: unknown) => {
      const apiResponse = response as { data?: { message?: string }; message?: string };
      const successMessage = apiResponse?.data?.message || apiResponse?.message || 'User deleted successfully!';
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              {(error as { message?: string })?.message || 'An error occurred while fetching users'}
            </p>
          </div>
        </div>
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <span>All Users ({pagination.totalRecord || 0})</span>
              {isLoading && (
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Are you sure you want to delete the user "{user.fullName}" ({user.email})?
            ...
            {!isLoading && filteredUsers.length > 0 && totalPages > 1 && (
              <div className="flex items-center justify-between mt-6 pt-6 border-t border-gray-200">
                <div className="text-sm text-gray-600">
                  Page {currentPage} of {totalPages} • {pagination.totalRecord} total users
                </div>
